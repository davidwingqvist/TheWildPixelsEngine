...
void Camera::Preset()
{
	...
	DirectX::XMStoreFloat4x4(&proj, DirectX::XMMatrixTranspose(
		DirectX::XMMatrixPerspectiveFovLH(0.5f * DirectX::XM_PI, (float)(Graphics::GetWidth() / Graphics::GetHeight()), 0.3f, 300.0f)));
...
void Camera::Update(DirectX::XMFLOAT3 pos, DirectX::XMFLOAT3 lookAt)
{
	...
	Graphics::GetContext()->Map(this->cameraBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	memcpy(mappedResource.pData, &this->parts, sizeof(CamParts));
	Graphics::GetContext()->Unmap(this->cameraBuffer, 0);
}
